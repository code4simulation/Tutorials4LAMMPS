...
### 10.3 액체 참조 시스템: Uhlenbeck-Ford(UF) 모델의 이론 및 구현

액체(Liquid)상의 자유 에너지를 계산할 때는 주로 격자가 없는 완전 유체를 참조 시스템으로 사용합니다. 문헌상 가장 널리 알려진 것은 **Lennard-Jones (LJ) 유체**이나, 실제 워크플로우 자동화에서는 **Uhlenbeck-Ford (UF) 모델**이 수치적 안정성과 해석적 편의성 덕분에 압도적으로 선호됩니다.

#### A. UF 모델의 물리적 정의 및 포텐셜 수식
UF 모델은 입자 간의 반발력이 원점에서 유한한 값을 가지는 **Soft-core** 포텐셜의 일종입니다. 

*   **포텐셜 에너지 ($V_{UF}$)**: 
    $$V_{UF}(r) = -\epsilon \ln\left[1 - \exp\left(-\left(\frac{r}{\sigma}\right)^2\right)\right]$$
    여기서 $\epsilon$은 에너지 스케일, $\sigma$는 길이 스케일입니다. 
*   **특징**: 
    - $r \to 0$에서 $V(r) \approx -2\epsilon \ln(r/\sigma)$ 로 로그함수적으로 발산하지만, $e^{-V/k_B T}$ 항은 0으로 수렴하여 적분 가능합니다.
    - $r \to \infty$에서 매우 빠르게 0으로 감쇄하여 컷오프 설정이 용이합니다.

#### B. 무차원 밀도와 열역학적 상태
UF 모델의 열역학적 거동은 무차원 밀도 $x$에 의해 결정됩니다. $x$는 제2 비리얼 계수($B_2$)와 수밀도($\rho$)의 곱으로 정의됩니다.
*   **제2 비리얼 계수 ($B_2$)**: UF 모델은 일반화된 포텐셜 지수 $p$를 가질 수 있으나, **$p=1$인 경우가 가장 표준적인 형태**입니다.
    - **$p=1$의 의미**: 포텐셜 수식 내의 거 거듭제곱 지수가 2(즉, $(r/\sigma)^2$)인 경우를 뜻하며, 이는 Mayer f-function이 가우시안(Gaussian) 형태를 띠게 되어 제2 비리얼 계수가 $B_2 = \frac{1}{2}(\pi \sigma^2)^{1.5}$ 로 매우 단순하고 깔끔하게 도출되는 수학적 이점을 가집니다.
    - 대부분의 자유 에너지 계산 라이브러리(`calphy` 등)는 이 $p=1$ 모델에 대해 정밀하게 피팅된 스플라인 데이터를 제공하므로, 실무적으로는 $p=1$이 기본값으로 고정됩니다.
*   **무차원 밀도 ($x$)**: 
    $$x = B_2 \rho = \frac{1}{2} (\pi \sigma^2)^{1.5} \frac{N}{V}$$
    이 식을 통해 실제 시스템의 밀도 $\rho$와 $\sigma$ 파라미터를 무차원 상태 $x$로 대응시킵니다.

#### C. 이상 기체 및 과잉 자유 에너지 ($F_{ex}$) 유도 과정

액체의 총 자유 에너지는 이상 기체 항($F_{ideal}$)과 분자 간 상호작용에 의한 과잉 항($F_{ex}$)의 합으로 정의됩니다: $F = F_{ideal} + F_{ex}$.

##### 1. 이상 기체 자유 에너지 ($F_{ideal}$) 계산
이상 기체의 헬름홀츠 자유 에너지는 통계역학적으로 다음과 같이 계산됩니다.
*   **분배 함수 ($Z_{ideal}$)**: $Z_{ideal} = \frac{V^N}{N! \Lambda^{3N}}$  (단, $\Lambda$는 열 탈브로이 파장)
*   **Stirling 근사($\ln N! \approx N \ln N - N$) 적용**:
    $$F_{ideal} = -k_B T \ln Z_{ideal} = N k_B T \left[ \ln(\rho \Lambda^3) - 1 \right]$$
    여기서 $\rho = N/V$는 수밀도입니다. 이 항은 입자 간 상호작용이 없는 순수 운동 에너지와 설정 엔트로피 기여분을 포함합니다.

##### 2. 과잉 자유 에너지 ($F_{ex}$) 유도 (Thermodynamic Integration)
실제 시스템의 압력을 이상 기체로부터의 편차인 압축 인자($Z$)를 통해 정의하면 유도 과정은 다음과 같습니다.
1.  **기본 열역학 관계식**: 정온 조건($dT=0$)에서 $dF = -P dV$ 입니다.
2.  **압력과 밀도의 관계**: $P = Z \rho k_B T = Z \frac{N}{V} k_B T$ 를 대입하면,
    $$dF = -Z \frac{N k_B T}{V} dV$$
3.  **변수 변환 ($V \to \rho$)**: $\rho = N/V$ 이므로 $d\rho = -(N/V^2) dV = -(\rho/V) dV$ 가 되어, $dV/V = -d\rho/\rho$ 입니다.
    $$dF = Z N k_B T \frac{d\rho}{\rho}$$
4.  **이상 기체와의 차이 ($F_{ex}$)**: 이상 기체의 경우 $Z=1$ 이므로 $dF_{ideal} = N k_B T \frac{d\rho}{\rho}$ 입니다. 따라서,
    $$dF_{ex} = dF - dF_{ideal} = (Z - 1) N k_B T \frac{d\rho}{\rho}$$
5.  **무차원 밀도 $x$로의 전환 및 적분**: $x = B_2 \rho$ 이므로 $dx/x = d\rho/\rho$ 입니다. 이를 $0$ (이상 기체)에서 현재 밀도 $x$까지 적분하면,
    $$\frac{F_{ex}}{N k_B T} = \int_0^x \frac{Z(x') - 1}{x'} dx'$$
    이 공식이 바로 `uf_math.py`에서 스플라인 데이터를 적분하여 자유 에너지를 산출하는 수학적 근거입니다.

#### D. LJ 대비 UF 모델의 장점
1.  **해석적 상태 방정식**: LJ와 달리 UF 모델은 $x \in [0, 4]$ 구간에서 매우 정밀하게 피팅된 범용 스플라인 테이블(`spline1`)이 존재하므로, 추가 시뮬레이션 없이 $F_{ex}$를 즉시 계산할 수 있습니다.
2.  **적분 안정성**: $r=0$ 부근에서 포텐셜이 급격하지 않아, TI 스위칭 시 입자 간 겹침(overlap)에 의한 수치적 발산 문제가 거의 발생하지 않습니다.
3.  **최적화 용이성**: $\sigma$ 값을 조절하여 타겟 액체의 RDF 첫 번째 피크와 유사하게 맞춤으로써 스위칭 중의 에너지 변화량($\Delta U$)을 최소화할 수 있습니다.

#### E. 구현 상의 주의사항
*   **$\epsilon$ 설정**: 시뮬레이션에서 $\epsilon = k_B T$로 설정하면 무차원 $Z(x)$와 직접 비교가 가능합니다. 
*   **밀도 범위**: `calphy`에서 제공하는 순정 스플라인 데이터는 $x \le 4.0$ 범위에서만 유효하므로 주의가 필요합니다. 다만, 본 워크플로우에서는 이를 **$x=10.0$까지 확장한 마스터 테이블(`uf_math_extended.py`)**을 별도로 구축하여 고밀도 액체 시스템에도 대응 가능하도록 하였습니다.

#### F. 마스터 테이블(Master Table) 확장 및 유동적 운영
초기 UF 데이터는 $x \in [0, 4]$ 구간으로 제한되어 있었으나, 본 프로젝트에서는 범용성을 높이기 위해 시뮬레이션을 통해 **$x=10.0$까지 마스터 테이블을 확장**하였습니다.

1.  **확장 데이터 생성**: `extend_uf_range.py`를 활용하여 고밀도 영역($x \in [4, 10]$)의 압력 데이터를 추가 수집하고, 기존 고정밀 데이터와 부드럽게 연결(Cubic Spline)하여 `uf_math_extended.py`를 생성하였습니다.
2.  **연속적 보간**: 하드코딩된 구간별 계수 대신 전체 구간에 대해 `scipy.interpolate.CubicSpline`을 사용하여 임의의 밀도에서 자유 에너지를 정확히 산출합니다.
3.  **동적 확장 기능**: 테이블 범위를 벗어나는 고밀도가 요청될 경우, `uf_math.expand_uf_table()` 메서드를 통해 LAMMPS 시뮬레이션을 자동 실행하고 마스터 테이블을 즉시 확장할 수 있는 인터페이스를 제공합니다.
4.  **시스템 독립성**: UF 모델은 무차원($x$) 모델이므로, 한 번 구축된 $x \in [0, 10]$ 마스터 테이블은 모든 액체 시스템에 공통적으로 적용 가능하여 매우 유동적인 작업이 가능합니다.
